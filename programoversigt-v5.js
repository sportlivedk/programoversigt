class SportLiveScheduleV5 extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
            <style>
                :host {
                ...
                #sl-selector-container {
                    ...
                    padding-bottom: 15px; 
                    width: 100%;
                    /* Fjernet transition på transform for at gøre scroll 100% responsiv og undgå hak */
                    transition: box-shadow 0.2s; 
                    position: relative;
                    will-change: transform; /* Optimerer ydeevnen i browseren */
                }
    ...
    setupStickyScroll() {
        ...
        // Vi sætter højden på indpakningen for at undgå indholdet hopper
        ...
        const onScroll = () => {
            ...
            // Beregn hvor langt toppen af vores element er under den faste WIX-header
            const offset = headerHeight - wrapperRect.top;
            
            // Beregn max grænsen, så menuen ikke forsvinder ned forbi bunden
            const maxOffset = widgetRect.height - container.offsetHeight;

            if (offset > 0 && offset < maxOffset) {
                // Fysisk flyt datovælgeren nedad (ligner en fastlåsning)
                container.style.transform = `translateY(${offset}px)`;
                container.style.zIndex = '99999';
                if (!container.classList.contains('is-sticky')) container.classList.add('is-sticky');
            } else if (offset >= maxOffset) {
                container.style.transform = `translateY(${maxOffset}px)`;
        ...
        // VIGTIGT: Aggressiv lytning ('true') på alle scroll-events og touch for mobiler
        window.addEventListener('scroll', () => window.requestAnimationFrame(onScroll), true);
        window.addEventListener('touchmove', () => window.requestAnimationFrame(onScroll), true);
    }
...
// LÆG MÆRKE TIL DET NYE NAVN HER:
customElements.define('sport-live-schedule-v5', SportLiveScheduleV5);